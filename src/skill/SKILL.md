...
- **Storage:** `ext.storage.local|sync|session` with `.get(key)` / `.set(obj)` / `.clear()`. Auto-reset between tests. `get(key)` returns the unwrapped value (not `{ key: value }`). Matchers: `await expect(ext.storage.local).toHaveStorageValue(key, expected)` (single read, supports asymmetric matchers); `await expect(ext.storage.local).toEventuallyHaveStorageValue(key, expected, opts?)` (polls until equal or timeout; `opts`: `{ timeout?=5000, interval?=100 }`; also works for key deletion with `expected: undefined`); `await expect(ext.storage.local).toHaveStorageKeys(keys)` (subset check — passes when all listed keys are present).
- **Windows:** `const win = await ext.windows.create(opts?)` — opens a real browser window seeded with tabs. `opts`: `{ tabs?: string[] (full URLs or bare extension paths), focused?: boolean (default true) }`. Returns `WindowHandle`: `{ id: number, tabs: Page[], focus(): Promise<void>, close(): Promise<void> }`. `win.tabs` contains Playwright `Page` handles for each seeded tab in order; empty when no `tabs` are supplied. Windows are torn down when the per-test context closes; `win.close()` is for mid-test cleanup. Throws `window/create-failed`.
- **Tabs:** `await ext.tabs.create(url, opts?)` → `Page` (url = full URL or bare extension path; `opts`: `{ windowId?, active? }`); `await ext.tabs.query(filter?)` → `TabInfo[]` (TabInfo = `{ id?, windowId?, url?, active: boolean, index: number }`); `await ext.tabs.close(tab)` (tab = `Page` matched by URL or numeric tab id; throws `tabs/not-found`).
