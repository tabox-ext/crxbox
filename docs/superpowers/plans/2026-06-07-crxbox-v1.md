...
Expected: install succeeds; `dist/index.js`, `dist/index.cjs`, `dist/index.d.ts` produced; typecheck passes.
...
export class BackgroundHelper {
  ...
  async waitForReady(timeoutMs = 10_000): Promise<void> {
    ...
    while (Date.now() < deadline) {
      try {
        const ok = await this.evaluate(() => typeof chrome !== 'undefined');
        if (ok) return;
      } catch (e) {
        lastError = e;
      }
  ...
  /** Forcefully terminate the MV3 service worker via CDP (not natural suspend). */
  async kill(): Promise<void> {
    const pages = this.ext.context.pages();
    const page = pages[0] ?? (await this.ext.context.newPage());
    const client = await this.ext.context.newCDPSession(page);
    await client.send('ServiceWorker.enable');
    // stopAllWorkers forces termination of all SWs in the browser context.
...
> **Risk note (highest-risk task):** forced SW termination is "the gap" crxbox fills and the exact CDP call is environment-sensitive. If `ServiceWorker.stopAllWorkers` does not terminate the worker in your Chromium, fall back to enumerating targets and closing the `service_worker` target: attach a browser-level session, `Target.setDiscoverTargets({discover:true})`, find the `type==='service_worker'` target whose URL starts with `ext.url('')`, then `Target.closeTarget({targetId})`. If kill behavior is still unclear, invoke **superpowers:systematic-debugging** before changing the test.
...
test('state written before a forced kill survives restart', async ({ ext }) => {
  ...
  const value = await ext.background.evaluate(async () => {
    return (await chrome.storage.local.get('survived')).survived;
  });
...
export class PopupHelper {
  ...
  /** open-for-correct-tab: drive the action popup from the SW so it binds to the active tab. */
  async openForTab(activeTab: Page, popupPath = 'popup.html'): Promise<Page> {
    ...
    await this.ext.background
      .evaluate(async () => {
        await chrome.action.openPopup();
      })
      .catch(() => {
        /* gesture/context constraints — surfaced below if no page appears */
      });
    return opened.catch(() => {
...
> **Honesty note (spec §popup two modes):** `chrome.action.openPopup()` has gesture/context constraints and may not yield a page in all headless configurations. `open()` covers the bulk of assertions; `openForTab()` validates active-tab wiring. If `openForTab` proves unreliable headless, the documented fallback is to run that single spec headed (`--headed`) — do not weaken the assertion.
...
Requires `@playwright/test` (peer) and `npx playwright install chromium`. Node 18+.
```
